...
const generateUpdateSceneState = (state, sceneData) =>
  applySpec({
    state: always(tags.UPDATE_OR_ADD_SCENE_TAG),
    scenes: pipe(prop('scenes'), assoc(sceneData.id, sceneData)),
  })(state);

const updateScene = (state) => ({ payload }) =>
  generateUpdateSceneState(state, payload);

const generateRemoveSceneState = (state, id) => ({
  state: tags.REMOVE_SCENE_TAG,
  scenes: pipe(prop('scenes'), dissoc(id))(state),
});

const removeScene = (state) => ({ payload: { id } }) =>
  generateRemoveSceneState(state, id);
...
const reducer = (state = initialState, action) => {
  return cond([
    [typeEquals(tags.UPDATE_OR_ADD_SCENE_TAG), updateScene(state)],
